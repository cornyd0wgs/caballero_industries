...
$next_number = (int) mysqli_fetch_assoc($count_result)['total'] + 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($conn, 'SELECT id FROM products WHERE product_code = ?');
        mysqli_stmt_bind_param($stmt, 's', $old['product_code']);
    ...
    $image_path = 'images/no-image.jpg'; // used if no file is uploaded
        ...
        if ($_FILES['image']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowed_extensions, true)) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $destination)) {
                    ...
                    $errors[] = 'Could not save the uploaded image. Check that images/products/ is writable.';
                }
            }
        }
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = mysqli_prepare(
            $conn,
            'INSERT INTO products (product_code, name, description, price, discount_price, quantity, image, is_featured, is_popular)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
        );
        mysqli_stmt_bind_param(
            $stmt, 'sssddisii',
            $old['product_code'], $old['name'], $old['description'],
            $price, $discount_price, $quantity, $image_path, $is_featured, $is_popular
        );
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
...
?>
    ...
    <section class="admin-section">
      <div class="container admin-form-container">
        ...
        <form class="admin-form" method="post" action="admin-add-product.php" enctype="multipart/form-data">
          ...
          <div class="form-row">
            <label for="image">PRODUCT IMAGE (optional — JPG/PNG/WEBP, up to 5MB)</label>
            <input type="file" id="image" name="image" accept=".jpg,.jpeg,.png,.webp">
          </div>

          <div class="form-row-checkboxes">
            <label class="checkbox-label"><input type="checkbox" name="is_featured" checked> Show in Featured Collection</label>
            <label class="checkbox-label"><input type="checkbox" name="is_popular"> Mark as Popular (shows in homepage carousel)</label>
          </div>

          <button type="submit" class="btn btn-primary">ADD PRODUCT <span class="arrow">→</span></button>
        </form>
      </div>
    </section>
